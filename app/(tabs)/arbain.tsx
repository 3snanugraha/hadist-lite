...
export default function ArbainScreen() {
  ...
  const { loading, error, getArbainHadith } = useHadith();
  ...
  const loadHadith = async (pageNumber: number) => {
    ...
    const endIdx = Math.min(startIdx + ITEMS_PER_PAGE, 42);
    
    const hadithPromises = Array.from(
      { length: endIdx - startIdx }, 
      (_, i) => getArbainHadith(startIdx + i + 1)
    );
  ...
  const loadMore = async () => {
    if (loadingMore || hadithList.length >= 42) return;
