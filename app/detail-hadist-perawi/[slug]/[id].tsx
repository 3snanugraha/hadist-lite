...
export default function HadithDetail() {
  ...
  const { loading, error, getHadithByPerawi, getRandomPerawi } = useHadith();
  ...
  const loadHadith = async () => {
    ...
    let result;
    if (id === 'acak') {
      result = await getRandomPerawi();
    } else {
      result = await getHadithByPerawi(String(slug), Number(id));
    }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      >
      <Text style={styles.hadithNumber}>{slug} #{id}</Text>
      <ArabicTextViewer 
        text={hadith.arab} 
        fontSize={24}
        textAlign="right"
